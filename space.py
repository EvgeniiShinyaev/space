...
def start_analysis():
    ...
    if file_path:
        ...
        messagebox.showwarning("Внимание", "Выберите изображение перед проведением анализа")
...
if __name__ == '__main__':
    ...
    instructions = [
        "1. Введите путь к изображению или используйте кнопку 'Обзор'.",
        "2. Нажмите на кнопку 'Провести анализ'."
    ]

    for i, instruction in enumerate(instructions):
        lbl = tk.Label(root, text=instruction, font=("Arial", 10))
        lbl.place(relx=0.01, rely=0.1 + i * 0.1)

    entry_path = tk.Entry(root, width=50)
    entry_path.place(relx=0.03, rely=0.35)

    btn_browse = tk.Button(root, text="Обзор", width=10, bg="#DDDDDD", command=select)
    btn_browse.place(relx=0.75, rely=0.40)

    start = tk.Button(root, text="Провести анализ", width=30, bg="#DDDDDD", command=start_analysis)
    start.place(relx=0.5, rely=0.55)
